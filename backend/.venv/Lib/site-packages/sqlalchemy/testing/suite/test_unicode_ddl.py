...
class UnicodeSchemaTest(fixtures.TablesTest):
    ...
    def define_tables(cls, metadata):
        ...
        t1 = Table(
            "unitable1",
            metadata,
            Column("mÃƒÂ©il", Integer, primary_key=True),
            Column("\u6e2c\u8a66", Integer),
            test_needs_fk=True,
        )
        t2 = Table(
            "UnitÃƒÂ©ble2",
            metadata,
            Column("mÃƒÂ©il", Integer, primary_key=True, key="a"),
            Column(
                "\u6e2c\u8a66",
                Integer,
                ForeignKey("unitable1.mÃƒÂ©il"),
                key="b",
            ),
            test_needs_fk=True,
        )
        ...
        if testing.against("sqlite"):
            t3 = Table(
                ...
                Column("UnitÃƒÂ©ble2_b", Integer, ForeignKey("UnitÃƒÂ©ble2.b")),
                Column(
                    "\u6e2c\u8a66_self",
                    Integer,
                    ForeignKey("\u6e2c\u8a66.\u6e2c\u8a66_id"),
            ...
            t3 = Table(
                ...
                Column("UnitÃƒÂ©ble2_b", Integer),
                Column("\u6e2c\u8a66_self", Integer),
                test_needs_fk=True,
            )

    def test_insert(self, connection):
        connection.execute(t1.insert(), {"mÃƒÂ©il": 1, "\u6e2c\u8a66": 5})
    ...
    def test_col_targeting(self, connection):
        ...
        eq_(row._mapping[t1.c["mÃƒÂ©il"]], 1)
        ...
        eq_(row._mapping[t3.c["UnitÃƒÂ©ble2_b"]], 1)
        eq_(row._mapping[t3.c["\u6e2c\u8a66_self"]], 1)

    def test_reflect(self, connection):
        connection.execute(t1.insert(), {"mÃƒÂ©il": 2, "\u6e2c\u8a66": 7})
        ...
        connection.execute(tt1.insert(), {"mÃƒÂ©il": 1, "\u6e2c\u8a66": 5})
        connection.execute(tt2.insert(), {"mÃƒÂ©il": 1, "\u6e2c\u8a66": 1})
        connection.execute(
            tt3.insert(),
            {
                "\u6e2c\u8a66_id": 1,
        ...
        eq_(
            connection.execute(tt1.select().order_by(desc("mÃƒÂ©il"))).fetchall(),
            [(2, 7), (1, 5)],
        )
        eq_(
            connection.execute(tt2.select().order_by(desc("mÃƒÂ©il"))).fetchall(),
            [(2, 2), (1, 1)],
        )
        eq_(
            connection.execute(
    ...
    def test_repr(self):
        ...
        eq_(
            repr(t),
            (
                "Table('Ã¦Â¸Â¬Ã¨Â©Â¦', MetaData(), "
                "Column('Ã¦Â¸Â¬Ã¨Â©Â¦_id', Integer(), "
                "table=<Ã¦Â¸Â¬Ã¨Â©Â¦>), "
                "schema=None)"
            ),
        )
