...
can be found on `CTAN <http://mirrors.ctan.org/info/knuth-pdf/texware/tftopl.pdf>`_.
	...
	>>> # Ã¢â‚¬Å“widthÃ¢â‚¬Â is always available, other metrics are available only when
	>>> # applicable. All values are relative to Ã¢â‚¬Å“designsizeÃ¢â‚¬Â.
	>>> tfm.chars[ord("g")]
...
class TFM:
    ...
    def _read(self, file):
        ...
        # Ã¢â‚¬Å“errorsÃ¢â‚¬Â in the input file, but we instead try to output the file as
        ...
        if sizes.lf != (
            ...
            raise TFMException("Subfile sizes donÃ¢â‚¬â„¢t add up to the stated total")
