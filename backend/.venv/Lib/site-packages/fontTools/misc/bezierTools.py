...
def approximateQuadraticArcLengthC(pt1, pt2, pt3):
    ...
    # abs(BezierCurveC[2].diff(t).subs({t:T})) for T in sorted(.5, .5Ã‚Â±sqrt(3/5)/2),
...
def approximateCubicArcLengthC(pt1, pt2, pt3, pt4):
    ...
    # abs(BezierCurveC[3].diff(t).subs({t:T})) for T in sorted(0, .5Ã‚Â±(3/7)**.5/2, .5, 1),
