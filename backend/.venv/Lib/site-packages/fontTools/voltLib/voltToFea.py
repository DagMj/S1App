...
* VOLT allows subtable breaks in any lookup type, but AFDKO feature file
  implementations vary in their support; currently AFDKOÃ¢â‚¬â„¢s makeOTF supports
  subtable breaks in pair positioning lookups only, while FontToolsÃ¢â‚¬â„¢ feaLib
...
class VoltToFea:
    ...
    def _gposLookup(self, lookup, fealookup):
        ...
        if isinstance(pos, VAst.PositionAdjustPairDefinition):
            for (idx1, idx2), (pos1, pos2) in pos.adjust_pair.items():
                ...
                # If not both are groups, use Ã¢â‚¬Å“enum posÃ¢â‚¬Â otherwise makeotf will
                # fail.
                enumerated = False
    ...
    def _gsubContextLookup(self, lookup, prefix, suffix, ignore, fealookup, chained):
        ...
        if isinstance(sub, VAst.SubstitutionReverseChainingSingleDefinition):
            # Reverse substitutions is a special case, it canÃ¢â‚¬â„¢t use chained lookups.
            for key, val in sub.mapping.items():
                if not key or not val:
                    path, line, column = sub.location
                    log.warning(f"{path}:{line}:{column}: Ignoring empty substitution")
    ...
    def _lookupDefinition(self, lookup):
        ...
        if "\\" in lookup.name:
            # Merge sub lookups as subtables (lookups named Ã¢â‚¬Å“base\subÃ¢â‚¬Â),
...
def main(args=None):
    ...
    parser.add_argument(
        "--traceback", action="store_true", help="DonÃ¢â‚¬â„¢t catch exceptions"
    )
