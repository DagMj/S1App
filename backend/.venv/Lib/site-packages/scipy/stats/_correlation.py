...
def _xi_statistic(x, y, y_continuous):
    ...
    # such that X(1) Ã¢â€°Â¤ Ã‚Â·Ã‚Â·Ã‚Â· Ã¢â€°Â¤ X(n)"
    j = np.argsort(x, axis=-1)
    j, y = np.broadcast_arrays(j, y)
    y = np.take_along_axis(y, j, axis=-1)

    # "Let ri be the rank of Y(i), that is, the number of j such that Y(j) Ã¢â€°Â¤ Y(i)"
    r = stats.rankdata(y, method='max', axis=-1)
    # " additionally define li to be the number of j such that Y(j) Ã¢â€°Â¥ Y(i)"
...
def _xi_std(r, l, y_continuous):
    ...
    # Ã¢Ë†Å¡nÃ‚Â·ÃŽÂ¾n(X, Y) Ã¢â€ â€™ N(0, 2/5) in distribution as n Ã¢â€ â€™ Ã¢Ë†Å¾"
    if y_continuous:  # [1] Theorem 2.1
        return np.sqrt(2 / 5) / np.sqrt(n)

    # "Suppose that X and Y are independent. Then Ã¢Ë†Å¡nÃ‚Â·ÃŽÂ¾n(X, Y)
    # converges to N(0, Ãâ€žÃ‚Â²) in distribution as n Ã¢â€ â€™ Ã¢Ë†Å¾
