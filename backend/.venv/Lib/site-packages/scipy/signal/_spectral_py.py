...
def lombscargle(
    ...
    *,
    ...
    ``y(Ãâ€°) = a*cos(Ãâ€°*x) + b*sin(Ãâ€°*x) + c``, where the fit is calculated for
    each frequency independently. This algorithm was developed by Zechmeister
    and KÃƒÂ¼rster which improves the Lomb-Scargle periodogram by enabling
    ...
    y : array_like
        ...
        to True.
    freqs : array_like
        Angular frequencies (e.g., having unit rad/s=2Ãâ‚¬/s for `x` having unit s) for
    ...
    .. [3] M. Zechmeister and M. KÃƒÂ¼rster, "The generalised Lomb-Scargle periodogram.
