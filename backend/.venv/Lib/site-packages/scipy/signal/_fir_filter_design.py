...
def firls(numtaps, bands, desired, *, weight=None, fs=None):
    ...
    # We also omit the 1/Ãâ‚¬ from both Q and b equations, as they cancel
    # during solving.

    # We have that:
    #     q(n) = 1/Ãâ‚¬ Ã¢Ë†Â«W(Ãâ€°)cos(nÃâ€°)dÃâ€° (over 0->Ãâ‚¬)
    # Using our normalization Ãâ€°=Ãâ‚¬f and with a constant weight W over each
    # interval f1->f2 we get:
    #     q(n) = WÃ¢Ë†Â«cos(Ãâ‚¬nf)df (0->1) = Wf sin(Ãâ‚¬nf)/Ãâ‚¬nf
    ...
    #     b(n) = 1/Ãâ‚¬ Ã¢Ë†Â« W(Ãâ€°)D(Ãâ€°)cos(nÃâ€°)dÃâ€° (over 0->Ãâ‚¬)
    # Using our normalization Ãâ€°=Ãâ‚¬f and with a constant weight W over each
    # interval and a linear term for D(Ãâ€°) we get (over each f1->f2 interval):
    #     b(n) = W Ã¢Ë†Â« (mf+c)cos(Ãâ‚¬nf)df
    #          = f(mf+c)sin(Ãâ‚¬nf)/Ãâ‚¬nf + mf**2 cos(nÃâ‚¬f)/(Ãâ‚¬nf)**2
    # integrated over each f1->f2 pair (i.e., value at f2 - value at f1).
    ...
    # Use L'Hospital's rule here for cos(nÃâ‚¬f)/(Ãâ‚¬nf)**2 @ n=0
    b[0] -= m * bands * bands / 2.
